...
#		print '	Done.'
		...
		return len(open(self.filename).readlines())
	...
	def extendnoref(self, n, fileout=None):
		...
		*******************************************************************************************************************************************"""	
		...
		for line in fd:
			parts = line.split('\t')
			fdw.write(parts[0]+'\t'+str(max(0,int(parts[1])-n))+'\t'+str(int(parts[2])+n)+'\n')
		fd.close()
		fdw.close()
...
#		pbar.finish()
	...
	def sortcountkeys(self, counts):
		...
		*******************************************************************************************************************************************"""	
		...
		keys = list(map(list,list(counts.keys())))
