...
def histo (alldata,xlab,ylab,outdir,legend=None):
    ...
    maximum=0
    for data in alldata:
        maximum = round(max(maximum, max(data)),0)
        print(maximum)
#    print 'MAXIMUM = '+str(maximum)
    bin = list(range(1,int(maximum+1),max(1,int(maximum/50))))  
    histograms = []
    ...
    ax.set_xticks([1]+list(range(0,int(maximum+1),int(maximum/10)+1)[1:]))
...
#    histo(dist,"#46a246",50,"Coverage","Count",outdir)
    ...
    #    p95=numpy.percentile(ndist, 95)
        ...
        ws.write(i+1,1,len(ndist)-len(ndist.nonzero()));ws.write(i+1,2,float(p25));ws.write(i+1,3,float(p50));ws.write(i+1,4,float(p75));ws.write(i+1,5,float(maximum));ws.write(i+1,6,float(minimum));
        ws.write(i+1,7,mean);
