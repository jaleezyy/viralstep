...
def count_lines(filename):
	...
	nlines = float(tmp.readline().split(' ')[0])
	...
	************************************************************************************************************************************************************"""
	...
	for i,bamfile in enumerate(bamlist):
		...
		for entry in ntcontent:
			gccontent[(entry.fields[0], int(entry.fields[1]), int(entry.fields[2]))] = float(entry.fields[-8])*100
		print('	Done.')		
			
		fig = pyplot.figure(figsize=(13,6))
		ax = fig.add_subplot(111)
		
		region_ids = list(coverage.keys())
...
#					print 'Processing ' +line+'\n' 
			...
			region_ids=[]
			for currentKey in list(coverage.keys()): # Pybedtools does not work with regions with zero length -> remove them (there are a few of them)
				if currentKey[1]!=currentKey[2]:
					region_ids.append(currentKey)
						
		
##		
##		fdw=open('gcContent.txt','w')	
